...
Los resultados me mostraron cosas muy interesantes. Los casos de uso principales, que son realmente el corazón del sistema, tenían una cobertura excelente (98-99% de líneas), lo que me tranquilizó mucho porque significaba que estaba probando tanto el flujo normal como todos los casos de error que se me habían ocurrido. La máquina de estados, que es súper crítica porque coordina todo el flujo de trabajo, tuvo una cobertura bastante satisfactoria (91.67%), aunque me costó más trabajo conseguir probar todas las transiciones posibles.

El servicio de inferencia fue donde me di cuenta de que las cosas se complicaban—solo conseguí un 72.51% de cobertura porque es increíblemente difícil simular todas las posibles situaciones que pueden pasar al procesar señales EEG reales y trabajar con modelos de machine learning. Ahí me di cuenta de que no todo se puede probar de manera automatizada.

La cobertura total del 81.38% me parecía bastante decente, y además se alineaba bien con los principios de la norma UNE-EN 62304 para software médico de Clase A, que básicamente dice que el nivel de pruebas tiene que ser proporcional al riesgo del software.
...
\subsubsection{Las pruebas manuales que me salvaron la vida}
...
La distribución de cobertura varió bastante según el tipo de componente, lo que al final tenía mucho sentido: mantuve niveles altos (98%) en la capa de dominio donde está toda la lógica crítica, mientras que los servicios llegaron a aproximadamente un 59%, los puertos un 54%, los adaptadores un 47% y otros componentes auxiliares un 29%. Esta distribución me parecía consistente con la estrategia de gestión de riesgos que había adoptado—priorizar la verificación exhaustiva de los componentes más críticos.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|p{6cm}|c|p{4cm}|}
        \hline
        \textbf{ID} & \textbf{Descripción} & \textbf{Severidad} & \textbf{Cómo lo resolví} \\
        \hline
    ...
    \caption{Anomalías que encontré y cómo las resolví}
    \label{tab:anomalies}
\end{table}
